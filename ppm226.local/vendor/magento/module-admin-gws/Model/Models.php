...
class Models extends \Magento\AdminGws\Model\Observer\AbstractObserver implements CallbackProcessorInterface
{
    ...
    public function cmsBlockSaveBefore($model)
    {
        ...
        $model->setData(
            'stores',
            $this->_forceAssignToStore($this->_updateSavingStoreIds($model->getData('stores'), $originalStoreIds))
        );
    }
    ...
    public function newsletterQueueSaveBefore($model)
    {
        ...
        if (!$storeIds || !$this->_role->hasStoreAccess($storeIds)) {
            throw new \Magento\Framework\Exception\LocalizedException(__('Please assign this entity to a store view.'));
        }
    ...
    public function catalogProductSaveBefore($model)
    {
        ...
        $this->checkStoreViewAccess($model);
     ...
     * @throws LocalizedException
     */
    private function checkStoreViewAccess(\Magento\Catalog\Model\Product $model)
    {
        if ($model->getId()
            ...
            throw new LocalizedException(
                __(
                    'You don\'t have permissions to save this item in scope of current store view. ' .
                    'Please, switch to allowed store view'
                )
            );
        }
    }
    ...
    public function salesOrderBeforeSave($model)
    {
        if (!$this->_role->hasWebsiteAccess($model->getStore()->getWebsiteId(), true)) {
            throw new LocalizedException(__('You can create orders only in an active store.'));
        }
    }
    ...
    protected function _forceAssignToWebsite($websiteIds)
    {
        if (count(array_intersect($websiteIds, $this->_role->getWebsiteIds())) === 0
            ...
            throw new \Magento\Framework\Exception\LocalizedException(__('Please assign this item to a store view.'));
        }
        return $websiteIds;
    }
    ...
    protected function _forceAssignToStore($storeIds)
    {
        if (count(array_intersect($storeIds, $this->_role->getStoreIds())) === 0 && count($this->_role->getStoreIds())
            ...
            throw new LocalizedException(__('Please assign this item to a store view.'));
        }
        return $storeIds;
    }
    ...
    {
        throw new LocalizedException(__('You need more permissions to save this item.'));
    }
    ...
    {
        throw new \Magento\Framework\Exception\LocalizedException(__('You need more permissions to delete this item.'));
    }
    ...
    {
        throw new \Magento\Framework\Exception\LocalizedException(__('You need more permissions to view this item.'));
    }
